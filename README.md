...
| `4` | 否定・不同意 | Disagreement / Negative Response |
...
- **アプローチ**: Pydantic スキーマ制約（Structured Outputs）による分類

##### 2. 堅牢性・実行時設計
- **並行実行**: `ThreadPoolExecutor(max_workers=10)`
- **リトライ制御**: 429 レートリミット等のエラーに対する指数バックオフ付き自動再試行（`attempts=10`, `initial_delay=10s`, `max_delay=100s`）
- **中間キャッシュ・レジューム機構**: 正常完了サンプルを `tmp/sample_{idx}.json` に即座に永続化し、中断時も未処理分から自動再開

##### 3. 実行時の様子および挙動分析
- **総所要時間**: 880.25 秒（約 14 分 40 秒）
- **実効スループット**: 約 3.92 samples/sec（毎分約 235 件）
- **リクエスト挙動と安定性**: 並行実行時のレートリミット（429）およびクラウドサーバー側キューイング遅延による指数バックオフ待機が発生。最大ギャップは 17.51 秒を記録したものの、自動再試行により 3,447 件全件が正常完走
- **レイテンシ特性**: 平均レイテンシ `2,104.59 ms`（中央値: `1,865.96 ms`, p90: `3,519.48 ms`, p99: `5,296.69 ms`）。3 秒超過サンプルは 641 件（18.6%）、5 秒超過サンプルは 51 件（1.5%）
- **予測確信度傾向**: 平均確信度 0.97（0.9658、中央値 0.98、最小 0.50）。確信度 0.95 以上が 3,100 件（89.9%）、0.98 以上が 2,381 件（69.1%）と大半で極めて高い確信度を出力（確信度 0.50 未満は 0 件）
...
##### 3. 実行時の様子および推論挙動分析
- **総所要時間**: 194.44 秒（約 3 分 14 秒）
- **実効スループット**: 約 17.73 samples/sec（毎分約 1,064 件、Gemini Flash Lite の約 4.5 倍）
- **セッション安定性**: 最大ギャップわずか 0.51 秒。並行10スレッド環境下でもレートリミットや途切れなく、クラッシュ・再起動ゼロで単一セッション完走
- **スパイク挙動と自動復旧**: 3 秒を超過したサンプルは全 3,447 件中わずか 8 件（0.23%、1 秒超過もわずか 12 件）。指数バックオフが自律的に稼働し、Fail-Fast 停止を起こさず自動復旧
- **モデル出力の確信度**: 平均確信度 0.94（中央値 1.00）。確信度 1.00 の完全確信が 2,392 件（69.4%）、0.90 以上が 2,903 件（84.2%）。確信度 0.50 未満はわずか 69 件（2.0%）
...
| モデル名 | 分類アプローチ | ROC-AUC (Macro) | Accuracy | Macro-F1 | 平均 (ms) | 最小値 (ms) | p25 (ms) | p50 (ms) | p75 (ms) | p90 (ms) | p99 (ms) |
| :--- | :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| LightGBM | TF-IDF + GBDT (ローカル) | 0.9945 | 0.9066 | 0.9081 | **7.76** | **3.44** | **5.00** | **6.16** | 9.33 | 13.43 | 21.55 |
| DistilBERT | Transformer Fine-tuning (ローカル GPU) | **0.9970** | **0.9315** | **0.9333** | 8.03 | 5.54 | 6.74 | 7.35 | **8.75** | **10.00** | **17.31** |
| Gemini 3.5 Flash Lite | 生成型 LLM API | 0.9751 | 0.9060 | 0.9058 | 2104.59 | 670.98 | 1234.84 | 1865.96 | 2729.76 | 3519.48 | 5296.69 |
| Jev (jev-1.13) | 判断特化 System One API | 0.9806 | 0.8784 | 0.8790 | 563.87 | 444.34 | 511.79 | 536.34 | 565.08 | 595.43 | 747.52 |
...
# API キーの設定
cp .env.sample .env
# .env を編集して GEMINI_API_KEY, TYPESAFE_API_KEY を設定
```
...
├── 03_gemini_flash_lite/        # Gemini Flash Lite モデル実験 
│   ├── input/                   # test.csv
